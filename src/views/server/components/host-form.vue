  ...
  >
    ...
    >
      <transition-group name="list" mode="out-in" tag="div">
        ...
        <el-form-item v-if="!isBatchModify" key="name" label="名称" prop="name">
          <el-input
        ...
        <el-form-item key="authType" label="认证方式" prop="authType">
          <el-radio v-model="hostForm.authType" value="privateKey"
            >密钥</el-radio
          >
          <el-radio v-model="hostForm.authType" value="password">密码</el-radio>
          <el-radio v-model="hostForm.authType" value="credential"
            >凭据</el-radio
          >
        </el-form-item>
        <el-form-item
        ...
        >
          <el-tooltip
            content="将在实例到期前7、3、1天发送提醒(需在设置中绑定有效邮箱)"
            placement="right"
          >
            <el-switch
...
import { RSAEncrypt, AESEncrypt, randomStr } from "@/utils/index";

const {
  proxy: { $api, $router, $message, $store }
} = getCurrentInstance();
...
const handleSave = () => {
  formRef.value.validate().then(async () => {
    ...
    if (isBatchModify.value) {
      ...
      if (Object.keys(updateFileData).length === 0)
      ...
      let newHosts = batchHosts.value
        ...
        .map(item => {
          ...
          if (item[authType])
            item[authType] = AESEncrypt(item[authType], tempKey);
      ...
      if (defaultData.value) {
        let { msg } = await $api.updateHost(
          Object.assign({}, formData, { oldHost: oldHost.value })
        );
        $message({ type: "success", center: true, message: msg });
      } else {
    ...
    visible.value = false;
    emit("update-list", {
      host: isBatchModify.value ? batchHosts.value : hostForm.value.host
    });
  });
};
</script>
